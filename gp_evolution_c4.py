...
from policies import GreedyPolicy, IntermediateGreedyPolicy, ImprovedGreedyPolicy, MinimaxPolicy
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_individuals', type=int, help='Seed for the experiment', default=9)
    parser.add_argument('--n_generations', type=int, help='Seed for the experiment', default=100)
    parser.add_argument('--greedy', type=int, help='Percentage for greedy strategy', default=25)
    parser.add_argument('--greedy_intermediate', type=int, help='Percentage for intermediate greedy strategy',
                        default=25)
    parser.add_argument('--min_max', type=int, help='Percentage for min-max greedy strategy',
                        default=25)
    parser.add_argument('--greedy_improved', type=int, help='Percentage for improved greedy strategy', default=25)
    ...
    n_greedy_improved_generations = int(n_generations * yellow_strategy["greedy_improved"] / 100)
    n_minmax_generations = n_generations - (n_greedy_generations + n_greedy_intermediate_generations + n_greedy_improved_generations)
    ...
    csv_logger = CSVLogger(
        filename=f"{dir_name}/{run_name}/res_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.csv",
    ...
    for _generation in tqdm(range(n_minmax_generations), position=1,
                            desc="Generations (Min Max Policy)"):
        start_eval = time.process_time()
        ...
        # Log metrics
        metrics = {
            "generation": _generation + n_greedy_generations + n_greedy_intermediate_generations + n_greedy_improved_generations,
            "max_fitness": max(fitnesses),
            "mean_fitness": np.mean(fitnesses),
            "max_dead_time": max(dead_times),
            "eval_time": eval_time,
    ...
    # Save final results
    jnp.save(f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.npy",
             genomes)
    jnp.save(f"{dir_name}/{run_name}/fitnesses_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.npy",
             fitnesses)
    with open(f"{dir_name}/{run_name}/config_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.yaml",
              "w") as file:
        yaml.dump(config, file)
    # Save final results also on a .txt format
    np.savetxt(f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.txt",
               np.array(genomes), fmt='%s')
    np.savetxt(f"{dir_name}/{run_name}/fitnesses.txt_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}",
               np.array(fitnesses), fmt='%s')

    # Render and evaluate the best genome
    # connect4_env.render()
    best_genome = genomes[jnp.argmax(fitnesses)]
    jnp.save(f"{dir_name}/{run_name}/best_genome_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['min_max']}.npy",
             best_genome)
