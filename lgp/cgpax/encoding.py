...
from lgp.cgpax.functions import function_switch, constants
...
def genome_to_lgp_program(genome: jnp.ndarray, config: Dict,
                          outputs_wrapper: Callable[[jnp.ndarray], jnp.ndarray] = jax.nn.relu) -> Callable:
    n_const = config["n_constants"]
    n_in = config["n_in"]
    n_out = config["n_out"]
    ...
    def _program(inputs: jnp.ndarray, register: jnp.ndarray) -> (jnp.ndarray, jnp.ndarray):
        ...
        register = jnp.concatenate([inputs, constants[:n_const], register[n_in:n_registers]])
