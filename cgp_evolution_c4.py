...
from cgpax.evaluation import evaluate_cgp_genome
...
def evaluate_genomes(genomes_array: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    with Pool(processes=os.cpu_count() - 1) as pool:
        for result in pool.starmap(evaluate_cgp_genome, args):
...
if __name__ == '__main__':
    ...
    dir_name = f'results_cgp/{args.policy_version}/results_{config["n_individuals"]}_{config["n_generations"]}_{config["adaptive"]}'
    ...
    n_greedy_expert_generations = n_generations - (
                n_greedy_generations + n_greedy_intermediate_generations + n_greedy_improved_generations)
    ...
    # Save final results
    jnp.save(
        f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
        genomes)
    jnp.save(
        f"{dir_name}/{run_name}/fitnesses_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
        fitnesses)
    with open(
            f"{dir_name}/{run_name}/config_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.yaml",
            "w") as file:
        yaml.dump(config, file)
    # Save final results also on a .txt format
    np.savetxt(
        f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.txt",
        np.array(genomes), fmt='%s')
    np.savetxt(
        f"{dir_name}/{run_name}/fitnesses_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.txt",
    ...
    jnp.save(
        f"{dir_name}/{run_name}/best_genome_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
        best_genome)
