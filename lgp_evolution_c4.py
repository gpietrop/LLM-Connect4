...
if __name__ == '__main__':
    ...
    parser.add_argument('--greedy_expert', type=int, help='Percentage for expert greedy strategy', default=0)
    ...
    config = {
        ...
        "yellow_strategy": {
            ...
            "greedy_expert": args.greedy_expert
        },
        "selection": {
            "elite_size": 5,
            "type": "tournament",
    ...
    dir_name = f'results_lgp/{args.policy_version}/results_{config["n_individuals"]}_{config["n_generations"]}_{config["adaptive"]}'
    ...
    n_greedy_expert_generations = n_generations - (n_greedy_generations + n_greedy_intermediate_generations + n_greedy_improved_generations)
    ...
    csv_logger = CSVLogger(
        filename=f"{dir_name}/{run_name}/res_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.csv",
    ...
    for _generation in tqdm(range(n_greedy_intermediate_generations), position=1,
        ...
        if config["adaptive"] and np.mean(percentages) > 0:
            # transfer the number of evaluation left to the other policy
            n_greedy_improved_generations = n_greedy_improved_generations + (n_greedy_intermediate_generations - _generation)
            break
    ...
    # Save final results
    jnp.save(f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
             genomes)
    jnp.save(f"{dir_name}/{run_name}/fitnesses_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
             fitnesses)
    with open(f"{dir_name}/{run_name}/config_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.yaml",
              "w") as file:
        yaml.dump(config, file)
    # Save final results also on a .txt format
    np.savetxt(f"{dir_name}/{run_name}/genotypes_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.txt",
               np.array(genomes), fmt='%s')
    np.savetxt(f"{dir_name}/{run_name}/fitnesses_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.txt",
               np.array(fitnesses), fmt='%s')

    # Render and evaluate the best genome
    # connect4_env.render()
    best_genome = genomes[jnp.argmax(fitnesses)]
    jnp.save(f"{dir_name}/{run_name}/best_genome_{yellow_strategy['greedy']}_{yellow_strategy['greedy_intermediate']}_{yellow_strategy['greedy_improved']}_{yellow_strategy['greedy_expert']}.npy",
             best_genome)
